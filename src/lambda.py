...
def generate_resource_list(ipaas_buckets, objects=False):
    resource_list = []

    for item in ipaas_buckets:
        if not objects:
            bucket_list.append("arn:aws:s3:::" + ipaas_bucket)
        else:
            object_list.append("arn:aws:s3:::" + ipaas_bucket + "/*")

    return bucket_list
    return object_list

def generate_buckets_policy(bucket_list, objects=False):
    if not objects:
        generate_resource_list(bucket_list)
        ipaas_bucket_policy = {
            "Version": "2012-10-17",
            "Statement": [
                {
...
return ipaas_bucket_policy

def generate_objects_policy(bucket_list, objects=True):
    generate_resource_list(object_list, objects=True)
    ipaas_object_policy = {
        "Version": "2012-10-17",
        "Statement": [
            {
